...
class IO {
     ...
     static async saveResponseInfo(response) {
          ...
          if (!response.surveyId)
               throw Error('cant save response without survey id')
     ...
     static async saveResponseAnswers(response) {
          await saveJson(
               `${ANSWERS_PATH}/${response._id}.json`,
               response.answers
          )
     }
     ...
     static async findUserByEmail(userEmail) {
          ...
          if (accounts) {
               ...
               if (account) {
                    const user = await IO.findUserById(account.userId)
                    return user
                    //  return new User(await IO.findUserById(account.userId))()
               }
          }
