...
class Survey extends Element {
                ...
                for (const question of page.questions) {
                    ...
                    switch (question.type) {
                        ...
                        case types.QUESTION_RADIO_GROUP:
                            for (
                                let i = 0;
                                i < question.content.choices.length;
                                i++
                            ) {
                                Language.translate(
                                    question.content.choices[i],
                                    language.code,
                                    word => {
    ...
    deleteUserById(userId) {
        let id = this.users.findIndex(
            (survey, index) => survey.userId == userId
        );
    ...
    static async loadSurveyResponseById(surveyId, responseId) {
        let response = await Response.loadSurveyResponseById(
            surveyId,
            responseId
        );
    ...
    static async generatReport(surveyId) {
        ...
        for (const question of questions) {
            ...
            switch (type) {
                ...
                case types.QUESTION_RATING:
                    ...
                    for (
                        let i = parseInt(content.min);
                        i <= parseInt(content.max);
                        i += 1
                    ) {
                ...
                switch (type) {
                    case types.ANSWER_RANGE:
                        ...
                        if (
                            !content.minValue ||
                            !content.maxValue ||
                            content.minValue > content.maxValue
                        ) {
                            devDeugger(answer, 'break');
                            break;
                        }
                        for (
                            let i = content.minValue;
                            i <= content.maxValue;
                            i += 1
                        )
                            if (tempReport[questionId].content[i] != null)
                    ...
                    case types.ANSWER_MULTIPLE_CHOICE:
                        ...
                        for (const choice of content.choices)
                            if (tempReport[questionId].content[choice] != null)
                                tempReport[questionId].content[choice]++;
                        break;
        ...
        devDeugger(tempReport);
        let report = {
            survey: _.pick(surveyInfo, 'title', 'description', 'color'),
            surveyId: surveyId,
            answers: [],
        };
        ...
        for (const question of questions) {
            report.answers.push({
                ..._.pick(question, '_id', 'type', 'title', 'description'),
                content: tempReport[question._id].content,
                answerType: tempReport[question._id].type,
            });
        }
        return report;
    }
                ...
                switch (answer.type) {
                    case types.ANSWER_RANGE:
                        if (answer.content.minValue <= answer.content.maxValue)
                            content = `${answer.content.minValue} -> ${
                                answer.content.maxValue
                            }`;
                        break;
                    case types.ANSWER_MULTIPLE_CHOICE:
...
async function test() {
    const wb = await Survey.generateResponsesExcelFile(
        '4cade2af-5a56-404f-97a0-89a6720ecc18'
    );
    wb.write('Excel.xlsx');
}
